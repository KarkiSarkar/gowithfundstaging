	...
	if ( $product->get_type() == 'crowdfunding' ) {
		if ( wpcf_function()->is_campaign_valid( $post->ID ) ) {
			...
			<form enctype="multipart/form-data" method="post" class="cart AA">
				...
				<button type="submit" class="<?php echo apply_filters( 'add_to_donate_button_class', 'wpneo_donate_button' ); ?>"><?php _e( 'Back Project', 'wp-crowdfunding' ); ?></button>
			</form>
			...
			if ( ! wpcf_function()->is_campaign_started() ) {
				...
				if ( wpcf_function()->is_reach_target_goal( $post->ID ) ) {
					_e( 'The project is successful.', 'wp-crowdfunding' );
				} else {
					_e( 'This project has been invalid or not started yet.', 'wp-crowdfunding' );
				}
			}
		}
	}
